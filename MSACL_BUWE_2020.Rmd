...
#- University of Virginia School of Medicine Department of Pathology
...
#tidyverse offers a specific function to do this:
replace_na(x,"Gretal")
```
...
lytes.wide <- pivot_wider(lytes.long, names_from = "Test", values_from = "Result" )
```
...
It is frequently necessary to jump back and forth between the traditional view, which we call "data wide" and the tidy view which we call "data long". This is accomplished with the functions `pivot_longer()` and `pivot_wider()`.
...
possum %>% filter(sex=='f') %>% mutate(foot_to_tail=footlgth/taill) %>%
  group_by(site, Pop) %>%
  summarize(mean_age=mean(age), median_foot_to_tail=median(foot_to_tail, na.rm=TRUE), count=n()) %>%
...
#the LIS downtime on Jan 29 is evident
```
...
There is so much more you can do with `ggplot()` that we won't have time to cover. If you are looking for more ways to display data and customize your plots, check out one of the many [ggplot 'cheat sheets'](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf) available online. You can also start by going through the data visialization chapter of Hadley Wickham's book [R for Data Science](https://r4ds.had.co.nz/data-visualisation.html) or see the [R Graphics Cookbook](https://r-graphics.org/) by Winston Chan. 
...
file_copy(path = my_list, new_path = "Data_Files_Copy") #Destination does not need to be a vector ... can be a directory
```
...
new_names <- str_replace(old_names, ".csv", str_c("-", Sys.Date(), ".csv", sep = ""))
...
## Option 1: Lab Utilization Investigation
...
library(dplyr)
library(ggplot2)
lab.data %>% mutate_if(is.character, as.factor) -> lab.data
lab.data %>% summary
...
#calculate the TAT column
lab.data %>% mutate(TAT = difftime(RESULT_DATE, SCHEDULED_DATE, units = "min")) -> lab.data

#calculate 50th and 90th percentiles
lab.data %>% 
  group_by(TEST) %>%
  summarize(perc50 = median(TAT), perc90 = quantile(TAT, probs = 0.9))

##### Break it up by location type and create a nice plot (perhaps boxplot or violin plot or time-series plots) of TAT for the Emergency room versus Inpatients.

#TAT visualization boxplot
lab.data %>%
  filter(LOCATION_TYPE == "Emergency" | LOCATION_TYPE == "Inpatient") %>%
  ggplot(aes(x = LOCATION_TYPE, y = as.numeric(TAT), fill = LOCATION_TYPE)) +
  geom_violin() + 
  geom_boxplot(width = 0.1, outlier.shape = NA) + 
  ylim(c(0,500))

#TAT visualization time series
lab.data %>%
  filter(LOCATION_TYPE == "Emergency" | LOCATION_TYPE == "Inpatient") %>%
  ggplot(aes(x = COLLECTION_DATE, y = as.numeric(TAT), col = LOCATION_TYPE)) +
  geom_point(size = 0.1) + 
  geom_smooth() +
  ylim(c(0,500))

##### Are there certain hours of the day when the turn around time suffers? Days of the week? Add hour of day and day of week

lab.data %>%
  mutate(hod = hour(COLLECTION_DATE),
         dow = weekdays(COLLECTION_DATE)) -> lab.data

#Examine by hour of day
lab.data %>%
  filter(LOCATION_TYPE == "Emergency" | LOCATION_TYPE == "Inpatient") %>%
  ggplot(aes(x = as.numeric(hod),y = as.numeric(TAT))) +
  geom_point() +
  geom_smooth() +
  ylim(c(0,500))

#Examine by day of week
lab.data %>%
  filter(LOCATION_TYPE == "Emergency" | LOCATION_TYPE == "Inpatient") %>%
  ggplot(aes(x = factor(dow), y = as.numeric(TAT))) +
  geom_violin() + 
...
# number of tests
lab.data %>%
  group_by(ORDERING_PHYSICIAN) %>%
  summarize(testcount = n()) %>% 
  arrange(desc(testcount))

# number of tests per patient
lab.data %>%
  group_by(ORDERING_PHYSICIAN) %>%
  summarize(testcount = n(), num_patients = length(unique(ID))) %>%
  mutate(countsperpatient = testcount/num_patients) %>%
  arrange(desc(countsperpatient))
...
# Some more specifics are possible - just for demonstration
  lab.data %>%
    group_by(ID) %>%
    summarize(Total = n()) -> Totals
...
library(dplyr)
library(tidyr)
fh <- read_excel("Data_Files/badly_entered_data.xlsx")
...
dates1 <- tibble(dmy_dates = fh$DOB[bad.dates.indices])
dates1 <- separate(dates1,
                   dmy_dates,
                   into = c("day","month","year"),
                   sep = "-")
dates1$year <- str_c("19",dates1$year)
dates1 <- str_c(dates1$year,dates1$month,dates1$day, sep = "-")

dates2 <- tibble(ymd_dates = fh$DOB[-bad.dates.indices])
dates2 <- str_c("19",dates2$ymd_dates, sep ="")
...
fh %>% 
  group_by(Sex) %>%
  summarize(median = median(Age, na.rm = TRUE), IQR = IQR(Age, na.rm = TRUE))
...
# Maybe by quantile of LDL?
library(gtools)
fh %>%
  mutate(quartile = quantcut(LDL, q = 4, na.rm = TRUE)) %>%
  filter(!is.na(quartile)) %>%
  ggplot(aes(x = quartile, y = `Lp(a)`, fill = quartile)) + 
  geom_boxplot() +
...
###Solution###
library(readr)
ms.out <- read_delim(file = "Data_Files/API_Flat_File.csv",
                   skip = 26,
                   delim = "\t")

#pull out only the columns of interest
ms.out <- tibble(Name = ms.out$`Sample Name`,
                 MRM = ms.out$`Analyte Peak Name`,
                 Conc = ms.out$`Calculated Concentration (pmol/L)`)

          
samples <- str_which(ms.out$Name,
                     pattern = "^E[0-9]{10}")

ms.out <- ms.out[samples,]

#pull out MRM1
MRM1 <- str_which(ms.out$MRM, pattern = "Aldo 1")
ms.out <- ms.out[MRM1,]

#convert conc to numeric
ms.out$Conc <- as.numeric(ms.out$Conc)
ms.out <- mutate(ms.out, Conc = replace_na(ms.out$Conc,0))
#alternatively
#ms.out$Conc[is.na(ms.out$Conc)] <- 0

output <- tibble("API", #replicates itself automatically
                 ms.out$Name,
                 "ALD", #replicates itself automatically
                 ms.out$Conc)

#write the processed file
write_csv(output,
          path = "processed_flat_file.txt",
          col_names = FALSE)
```

# Appendix: Old Skool Base R Plotting
...
```{r, include = FALSE}
###Solution###
T.data <- read_csv("Data_Files/testo_compare.csv")
library(mcr)
